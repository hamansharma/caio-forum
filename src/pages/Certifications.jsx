...
import { ArrowLeft, ArrowRight, ArrowUpRight, Award, BookOpenCheck, CheckCircle2, Filter, Flag, History, Info, Map, Search, SlidersHorizontal, Scale, X } from 'lucide-react';
...
export default function Certifications() {
  ...
  return <main className="certifications-page">
    ...
    {showPathwayHistory && <CertificationPathwayHistory onClose={() => setShowPathwayHistory(false)} />}
...
function CertificationPathway({ onClose }) {
  const [form, setForm] = useState({ targetRole: '', domain: '', experience: 'New' }); const [result, setResult] = useState(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(false); const [usage, setUsage] = useState(null); const [confirming, setConfirming] = useState(false);
  ...
  const build = async () => { setLoading(true); setError(''); try { const token = await auth.currentUser.getIdToken(); const response = await fetch('/api/certifications?action=pathway', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(form) }); const body = await response.json(); if (!response.ok) throw new Error(body.error); setResult(body); setUsage(current => ({ ...(current || {}), remaining: body.remaining, used: 2 - body.remaining })); setConfirming(false); } catch (err) { setError(err.message || 'Unable to build your pathway.'); setConfirming(false); } finally { setLoading(false); } };
...
function CertificationPathwayHistory({ onClose }) {
  ...
  return <div className="cert-detail-overlay" onClick={onClose}><section className="cert-suggestion cert-pathway" onClick={event => event.stopPropagation()}><button className="cert-close" onClick={onClose}><X size={19} /></button><h2>Your path history</h2><p>Completed pathway recommendations are saved privately to your account.</p>{loading && <p className="cert-form-status">Loading your saved paths…</p>}{error && <p className="cert-form-status">{error}</p>}{!loading && !error && (!paths.length ? <p className="cert-form-status">No saved paths yet. Build your first path from the Navigator.</p> : <section className="cert-path-history">{paths.map(path => <article key={path.id}><span>{new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric', year: 'numeric' }).format(new Date(path.createdAt))}</span><h3>{path.targetRole}</h3><p>{path.domain} · {path.experience}</p><ol>{(path.recommendations || []).map(item => <li key={item.id}>{item.name}</li>)}</ol></article>)}</section>)}</section></div>;
}

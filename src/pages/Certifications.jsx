...
import { ArrowLeft, ArrowRight, ArrowUpRight, Award, BookOpenCheck, CheckCircle2, Filter, Flag, Map, Search, SlidersHorizontal, Scale, X } from 'lucide-react';
...
export default function Certifications() {
  ...
  return <main className="certifications-page">
    <section className="certifications-hero"><span><Award size={15} /> CAIO Leadership Lab</span><h1>Certification Navigator</h1><p>Find credible credentials across technology, finance, education, operations, and more—then compare requirements, cost, authority, and renewal commitment before you invest.</p><div className="cert-hero-actions"><button className="cert-suggest-button" onClick={() => user ? setShowSuggestion(true) : setShowAuth(true)}>Suggest a certification <ArrowUpRight size={15} /></button><button className="cert-suggest-button" onClick={() => user ? setShowPathway(true) : setShowAuth(true)}><Map size={15} /> Build my path</button></div></section>
    ...
    <section className="cert-compare-bar"><div><Scale size={17} /><strong>Compare credentials</strong><span>Select up to three to weigh the commitment side by side.</span></div><div className="cert-compare-actions"><button className="cert-compare-reset" onClick={() => setComparison([])} disabled={!comparison.length}>Reset</button><button onClick={() => setComparisonOpen(true)} disabled={!comparison.length}>Compare {comparison.length}/3 <ArrowRight size={15} /></button></div></section>
    ...
    {showPathway && <CertificationPathway onClose={() => setShowPathway(false)} />}
...
function CertificationPathway({ onClose }) {
  const [form, setForm] = useState({ targetRole: '', domain: '', experience: 'New' }); const [result, setResult] = useState(null); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  const submit = async event => { event.preventDefault(); setLoading(true); setError(''); try { const token = await auth.currentUser.getIdToken(); const response = await fetch('/api/certifications?action=pathway', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(form) }); const body = await response.json(); if (!response.ok) throw new Error(body.error); setResult(body); } catch (err) { setError(err.message || 'Unable to build your pathway.'); } finally { setLoading(false); } };
  return <div className="cert-detail-overlay" onClick={onClose}><section className="cert-suggestion cert-pathway" onClick={event => event.stopPropagation()}><button className="cert-close" onClick={onClose}><X size={19} /></button><h2>Build my certification path</h2><p>Two free pathway requests per day. Recommendations are ranked from this verified catalog—not generated from a generic prompt.</p><form onSubmit={submit}><label>Target role<input required value={form.targetRole} placeholder="e.g., Cloud security engineer" onChange={event => setForm(current => ({ ...current, targetRole: event.target.value }))} /></label><label>Primary domain<select value={form.domain} onChange={event => setForm(current => ({ ...current, domain: event.target.value }))} required><option value="">Choose a domain</option>{certificationCategories.map(item => <option key={item}>{item}</option>)}</select></label><label>Experience<select value={form.experience} onChange={event => setForm(current => ({ ...current, experience: event.target.value }))}><option>New</option><option>Some experience</option><option>Experienced</option></select></label>{error && <p className="cert-form-status">{error}</p>}<button className="cert-official-link" disabled={loading}>{loading ? 'Building…' : 'Build pathway'}</button></form>{result && <section className="cert-path-results"><p>{result.rationale} {result.remaining} free request remaining today.</p>{result.recommendations.length ? result.recommendations.map((item, index) => <article key={item.id}><strong>{index + 1}. {item.name}</strong><span>{item.authority} · {item.level}</span><p>{item.description}</p></article>) : <p>No strong catalog match yet. Try a broader target role or domain.</p>}</section>}</section></div>;
}

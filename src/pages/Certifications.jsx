...
import { ArrowUpRight, Award, BookOpenCheck, CheckCircle2, Filter, Search, SlidersHorizontal, X } from 'lucide-react';
...
export default function Certifications() {
  ...
  const { user } = useForum();
  const [catalog, setCatalog] = useState(certifications);
  const [showSuggestion, setShowSuggestion] = useState(false);
  const [showAuth, setShowAuth] = useState(false);

  useEffect(() => {
    let active = true;
    fetch('/api/certifications').then(async response => {
      if (!response.ok) return null;
      return response.json();
    }).then(body => {
      if (active && body?.entries?.length) setCatalog(body.entries);
    }).catch(() => {});
    return () => { active = false; };
  }, []);

  const results = useMemo(() => catalog.filter(item => {
    const searchable = [item.name, item.authority, item.category, ...item.skills, ...item.targetRoles].join(' ').toLowerCase();
    return (!query || searchable.includes(query.toLowerCase())) && (category === 'All' || item.category === category) && (level === 'All' || item.level === level);
